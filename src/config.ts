...
const schema = z.object({
  ...
  API_PUBLIC_ENDPOINT: z.url().default("http://localhost:8080"),
  ...
  AUTH_MFA_STEP_UP_TTL_SEC: z.coerce.number().int().min(60).max(3600).default(300),
  AUTH_MFA_TOTP_WINDOW: z.coerce.number().int().min(0).max(2).default(1),
  AUTH_MFA_RECOVERY_CODE_COUNT: z.coerce.number().int().min(6).max(20).default(10),
  ...
  ASSET_CLEANUP_INTERVAL_SEC: z.coerce.number().int().positive().default(3600),
  ASSET_ORPHAN_GRACE_SEC: z.coerce.number().int().positive().default(86400),
