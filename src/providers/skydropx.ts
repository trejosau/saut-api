...
async function accessToken(): Promise<string> {
  ...
  const form = new URLSearchParams({
    ...
    grant_type: config.SKYDROPX_GRANT_TYPE,
  });
...
function nestedRecord(value: unknown, ...path: string[]): JsonRecord {
  return path.reduce<JsonRecord>(
    (record, segment) => toRecord(record[segment]),
    toRecord(value),
  );
}
...
export function ratesFrom(payload: unknown, quotationId?: string): ShippingQuote[] {
  ...
  return candidates
    ...
    .filter((rate) =>
      rate.quote_id.length > 0 && Number.isFinite(rate.price_mxn) && rate.price_mxn >= 0,
    );
}
...
export async function quoteNational(
  address: JsonRecord,
  orderId?: string,
): Promise<ShippingQuote[]> {
  if (config.SKYDROPX_MODE === "mock") {
    return [{
      quote_id: "national-standard",
      provider: "skydropx",
      service: "standard",
      price_mxn: config.NATIONAL_SHIPPING_COST_MXN,
      eta_days: 5,
    }];
  }
  ...
  const response = await fetchExternal(`${config.SKYDROPX_BASE_URL}/api/v1/quotations`, {
    ...
    body: JSON.stringify({ quotation: {
      address_from: {
        country_code: config.SHIP_FROM_COUNTRY_CODE,
        postal_code: config.SHIP_FROM_POSTAL_CODE,
        area_level1: config.SHIP_FROM_AREA_LEVEL1,
        area_level2: config.SHIP_FROM_AREA_LEVEL2,
        area_level3: config.SHIP_FROM_AREA_LEVEL3,
      },
      address_to: destination(address),
      parcels: [{
        weight: config.DEFAULT_PARCEL_WEIGHT_KG,
        length: config.DEFAULT_PARCEL_LENGTH_CM,
        width: config.DEFAULT_PARCEL_WIDTH_CM,
        height: config.DEFAULT_PARCEL_HEIGHT_CM,
      }],
      requested_carriers: config.SKYDROPX_REQUESTED_CARRIERS
        ? config.SKYDROPX_REQUESTED_CARRIERS.split(",").map((value) => value.trim()).filter(Boolean)
        : null,
  ...
  const quotationId = String(
    payloadRecord.id ?? nestedRecord(payloadRecord, "data").id ?? "",
  ) || undefined;
...
export async function createNationalShipment(
  ...
  if (config.SKYDROPX_MODE === "mock") {
    ...
    return {
      provider_shipment_id: id,
      tracking_number,
      tracking_carrier: "Skydropx",
      tracking_url: `https://mock.skydropx.local/track/${tracking_number}`,
      label_url: `https://mock.skydropx.local/labels/${id}.pdf`,
      raw: { mode: "mock" },
    };
  }
  ...
  const response = await fetchExternal(`${config.SKYDROPX_BASE_URL}/api/v1/shipments/`, {
    ...
    body: JSON.stringify({ shipment: {
      rate_id: rateId,
      printing_format: input.printing_format ?? "standard",
      address_from: {
      ...
      address_to: {
        ...
        reference: address.reference ?? address.line2 ?? null,
        ...destination(address),
      },
      packages: [{
        package_number: 1,
        package_protected: false,
        declared_value: input.declared_value ?? order.total_mxn,
        consignment_note: input.consignment_note ?? null,
  ...
  const payloadRecord = toRecord(payload);
  const provider_shipment_id = String(
    nestedRecord(payloadRecord, "data").id ?? payloadRecord.id ?? "",
  );
  ...
  return {
    ...
    tracking_number,
    tracking_carrier: String(findDeep(payload, "carrier_name") ?? "Skydropx"),
    tracking_url: findDeep(payload, "tracking_url_provider") ?? null,
    label_url: findDeep(payload, "label_url") ?? null,
    raw: payload,
  };
}
...
export async function fetchTracking(
  ...
  if (config.SKYDROPX_MODE === "mock") {
    return {
      delivered: false,
      events: [{ status: "in_transit", description: "En tránsito", happened_at: new Date().toISOString() }],
      raw: { mode: "mock" },
    };
  }
  ...
  const events = data.map((candidate): TrackingEvent => {
    ...
    return Object.keys(toRecord(item.attributes)).length > 0
      ? toRecord(item.attributes)
      : item;
  });
