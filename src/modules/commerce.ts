...
export async function registerCommerce(app: FastifyInstance, context: AppContext): Promise<void> {
  ...
  app.post<{Params:{cart_id:string}}>("/cart/sessions/:cart_id/items/predesigned",async(request,reply)=>{
    const body=asObject(request.body);const publication=(await context.database.query("select * from publications where slug=$1 or id=$2 limit 1",[body.publication_slug,body.publication_id??null])).rows[0];if(!publication)throw new HttpError(404,"Publicación no encontrada");
  ...
  app.get("/orders/lookup",async(request)=>{const query=asObject(request.query);const email=String(query.email??"").toLowerCase();const orders=(await context.database.query("select * from orders where lower(customer_email)=$1 order by created_at desc",[email])).rows;const order=orders.find(row=>row.id.replaceAll("-","").slice(0,8).toUpperCase()===String(query.order_code??"").replaceAll("-","").toUpperCase());if(!order)throw new HttpError(404,"Pedido no encontrado");return{id:order.id,email:order.customer_email,phone:order.customer_phone,status:order.status,shipping_method:order.shipping_method,order_code:order.id.replaceAll("-","").slice(0,8).toUpperCase()};});
